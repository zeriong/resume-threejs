...
export default class ContentsController {
  ...
  #init() {
    ...
    this.#returnToOrbitBtn.addEventListener("click", () =>
      this.#returnToOrbit()
    );
    ...
    this.#prevBtn.addEventListener("mouseenter", () => {
      ...
      if (this.#isActivePrev)
        this.#prevBtn.style.backgroundColor = "rgb(95, 104, 110)";
    });
  ...
  toContent(content, isBtn) {
    ...
    gsap.to(app.camera.instance.position, {
      ...
      onComplete: () => {
        ...
        if (app.windowSizes.width <= 497) {
          ...
          this.#lastUpdated.style.opacity = 0;
        } else {
        ...
        if (["projects", "roadmap", "skills"].includes(content.name))
          this.#webgl.style.zIndex = -1;
      },
    });

    gsap.to(app.camera.orbitControls.target, {
      ...content.controlsTarget,
      duration: 1,
      ease: "power1.inOut",
    });
    ...
    gsap.to(world.environment.lightParticleMaterial, {
      ...
      opacity: 0,
      onStart: () =>
        (world.environment.lightParticleMaterial.transparent = true),
    });
  }
  ...
  #toNext() {
    ...
    if (nextContent.name === "guestBook") {
      ...
      this.#breakTimeoutk(raycaster.timeout1, () =>
        raycaster.controlPopup("hidden")
      );
      raycaster.timeout1 = null;
      this.#breakTimeoutk(raycaster.timeout2, () =>
        raycaster.controlPopup("hidden")
      );
  ...
  #returnToOrbit() {
    ...
    this.#breakTimeoutk(app.raycaster.timeout1, () =>
      app.raycaster.controlPopup("hidden")
    );
    app.raycaster.timeout1 = null;
    this.#breakTimeoutk(app.raycaster.timeout2, () =>
      app.raycaster.controlPopup("hidden")
    );
    app.raycaster.timeout2 = null;
    this.#breakTimeoutk(this.#typingTimeout, () =>
      setTimeout(() => (this.#dialogContent.innerHTML = ""), 1000)
    );
    ...
    gsap.to(app.camera.instance.position, {
      ...
      onStart: () => {
        ...
        if (this.currentContent.name === "poster") {
          ...
          this.#lastUpdated.style.opacity = 0.7;
          // 기존 속성으로
          setTimeout(() => {
    ...
    gsap.to(world.environment.lightParticleMaterial, {
      ...
      opacity: 1,
      onComplete: () =>
        (world.environment.lightParticleMaterial.transparent = false),
    });
  }
