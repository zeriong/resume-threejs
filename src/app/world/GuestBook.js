...
export default class GuestBook {
  ...
  #init() {
    ...
    document.querySelector("#guestBookBtn").addEventListener("click", (e) => {
      ...
      if (!this.#isShowModal) {
        ...
        if (contentsController.isInGuestBook)
          return this.#controlReviewModal("on", 10);
  ...
  #setGuestReviews() {
    (async () => {
      ...
      rightArrowGeometry.setAttribute(
        "position",
      ...
      const rightArrowMesh = new THREE.Mesh(
        rightArrowGeometry,
        rightArrowMaterial
      );
  ...
  #createGuestReview() {
    ...
    if (name.value.length < 2 && message.value.length < 2)
      return alert("이름과 메시지는 2글자 이상으로 입력해주세요.");
  ...
  #paintGuestReview(isFirstLoad) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (isFirstLoad) {
        ...
        reviewMesh.rotation.x = THREE.MathUtils.degToRad(
          Math.random() * 7 - 3.5
        );
      ...
      if (review) {
        // 방명록 각도 변경
        this.#reviewMeshList[i].rotation.x = THREE.MathUtils.degToRad(
          Math.random() * 7 - 3.5
        );
        // canvas painting
  ...
  async #getFirestoreData(type) {
    ...
    const queryArr = [];
    if (type === "next")
      queryArr.push(startAfter(this.#lastVisible), limit(this.#limit));
    if (type === "prev")
      queryArr.push(endBefore(this.#firstVisible), limitToLast(this.#limit));
